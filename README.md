...
Scripts, configuration files, and ROOT macros for the full calibration
and analysis chain of the SuperNEMO calorimeter using ²??Bi calibration
sources, developed and tested on CC-IN2P3.

The repository contains two independent parts:

- **REAL** ? full pipeline for real experimental data: calibration constant
  extraction and their application to the same dataset, for all four energy
  correction modes. Uses Falaise 5.1.10.

- **SIMU** ? simulation pipeline for ²??Bi calibration events, used to study
...
Contains the `flreconstruct` configuration files for each stage of the
unified pipeline, applicable to both real and simulated data.
See `pipelines.drawio.png` for the full pipeline scheme.

| File | Pipeline stage | Input ? Output | Data type |
|------|---------------|----------------|-----------|
| `1D-pipeline.conf` | 1D-pipeline | UDD ? pCD ? CD (incomplete, no energy) | Real only |
...
Each file differs only in which energy corrections are applied and which
CSV file with calibration constants is used.

| File | Corrections applied |
|------|---------------------|
| `charge2energy-NOCALIB.conf` | None (raw energy only) |
| `charge2energy-OPTICAL.conf` | Optical correction only |
| `charge2energy-BOTH.conf` | Optical + Birks?Cherenkov (best estimate) |
...
- `calibration_path` ? path to the CSV file with $a_j$, $b_j$ constants

### SNCuts-pipeline.conf ? calibration event selection cuts

| Parameter | Value | Description |
|-----------|-------|-------------|
| `useEventHasVertexCloseToCalibSource` | `true` | Vertex within ellipse around nearest source |
| `source_cut_ellipse_Y` | 15 mm | Ellipse semi-axis in Y |
| `source_cut_ellipse_Z` | 40 mm | Ellipse semi-axis in Z |
| `hasNumberofKinks` | `1 0` | Reject kinked tracks ? first number ? number of straight tracks, second ? with 1 kinck; can be expanded ? third ? with 3 kincks ...|
| `useEventTrackHasOneAssocCaloHit` | `true` | Track must have exactly 1 associated OM hit |
| `minSumEnergy` / `maxSumEnergy` | 0 / 3500 keV | Energy range (example)|
...
2. **ChargedParticleTracker** ? vertex extrapolation and particle
   identification ? produces PTD bank
3. Gamma clustering commented out (not needed for single-electron
...
Each correction mode folder (`NOCALIB`, `OPTICAL`, `BETE`, `BOTH`)
follows the same structure. The compiled CalibrationTools for each mode
is located in the corresponding `Calibration_Constants_Extraction-<MODE>/`
subfolder under `REAL/`.
...
# Output: *_PTD_calibrationcuts.root (one per run)
...
# Merges per-run ROOT files preserving OM numbering
...
# Optional: select 600?800 keV events for inter-peak plateau study
...
# Output: *_PTD_c2e-SNCUTS.root (one per run)
```
...
| `REAL/BETE/calib_constants-BETE.csv` | Birks?Cherenkov only |
...
Configuration files and scripts for simulating ²??Bi calibration events
and processing them through the full reconstruction chain
...
calibration spectrum observed in real data: by tagging electrons that
deposited energy in the copper calibration source frame (GID 1066),
it was confirmed that these electrons are responsible for the broad plateau
between the 482 keV and 976 keV conversion peaks, and that their
contribution increases for electrons emitted at grazing angles
relative to the source foil.
...
| `SNCuts_600-800.conf` | SNCuts selecting only 600?800 keV events |
...
# e.g.: bash SIMU/submit_simu_array.sh 1 100
# Worker: simu.sh
...
# Single event visualisation in 600?800 keV window
...
# ? energy spectrum of electrons with non-zero deposit in copper frame (GID 1066)
...
Modified MiModule scripts that additionally extract energy deposits in
the copper calibration source frame (geometry identifier GID 1066).
Used to confirm that copper-frame energy losses are responsible for
the broad inter-peak plateau.
...
General-purpose ROOT macros and shell scripts, maintained as the most
recent versions. Can be used as templates or copied into a specific
correction mode folder.
...
| `send_600-800_one_ev.sh` | Single event in 600?800 keV window for `flvisualize` |
